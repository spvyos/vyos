...
should_I_check_cache<-function(cache = TRUE ){
  return ( ( exists("GlobalCacheOption"  ,
                    envir =rlang::global_env()) && get( "GlobalCacheOption")  == T ) && cache )
}
...
get_input_from_user<-function(prompt = "Enter any number : " , as.int = T , default.when.testing = 1 ){
    if( check_package_installed("testthat" ) &&  testthat::is_testing() ) return( default.when.testing )
...
which_cache_folder <- function(ans = 1 ){
    ...
    list_a <- list(  "1"=  "./caches_vyos" ,
                     "2" = tempdir() ,
                     "3" = "null"   )

    if(!toString(ans) %in% names( list_a )) return("null")
...
cache_folder_is_valid<-function(folder = "null") {
    ...
    if( to_string( folder ) == "null" ) return (FALSE)

     is.character(folder ) && length(folder )  ==  1
}
#--------------------------------------------------------------------save_cache
save_cache <- function(name , data) {
  ...
  if(!cache_folder_is_valid( cache_folder)) return(data)

  create_dir_if_not(cache_folder)
  success(SYMB_CACHE_SAVED )
  try({
...
check_cache <- function(name ) {
  ...
  if(!cache_folder_is_valid( cache_folders )) return( false )
...
load_cache <- function(name) {
  ...
  if(!cache_folder_is_valid( cache_folders )) return( NA )

  name <- vector_patch_cache_name( name )
  for (folder in cache_folders) {
    if (file.exists(file.path(folder, name_format_cache(name)))) {
      success(SYMB_CACHE_LOAD )
      okunan <- NA
      try({
...
weekly_cache <- function() {
  return ( sprintf( "%s_%s"  , format(as.POSIXct(Sys.time()) , "%Y") ,
