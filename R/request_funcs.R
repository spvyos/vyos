...
create_url_for_series_fred<-function(currentObj){
    ...
    paste0(currentObj$url , currentObj$observations_url , "?"
...
check_series_ID_for_dots<-function(currentObj , urlParts ){
    ...
    if( ! ( is_( .base  , "series" ) && is_( .source   , "evds" )   ) ) return(urlParts )
    .f<- function(a ){
        gsub( "_" , "." , a ,  fixed = T   )
    }
...
get_freq_number_evds<-function(freq ){
    ...
    freq_unify<- function( string ){
        .liste <- list( m = "month" ,
                        y = "year" ,
                        q = "quarter" ,
                        "null" =  "day" ) # series will default to most freq possible so
...
check_proxy_setting <- function(requires_proxy = T  , die_for_test = F ){
  if(!requires_proxy) return (invisible(T))
...
requestNow <- function(currentObj) {
  ...
  list_fncs <- list(evds           = request_httr2 ,
